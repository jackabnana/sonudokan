...
def activate_shop(request, pk):
    ...
    if shop.owner.id == request.user.id:
        ...
        shop.save()
        messages.success(
            request, f'Your brand is activated again!')
    return redirect(reverse('core:dashboard'))
...
class ShopCreateView(LoginRequiredMixin, generic.CreateView):
    ...
    def form_valid(self, form):
        ...
        object.save()
        messages.success(
            self.request, f'Your new brand was successfully added to TradeNpl.')
...
class ShopEditView(LoginRequiredMixin, generic.UpdateView):
    ...
    def form_valid(self, form):
        form.save()
        messages.success(
            self.request, f'Your brand is successfully edited')
        return redirect(reverse(self.success_url))
